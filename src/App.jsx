...
function BulkImportModal({onClose,onImport,userId}) {
  ...
  const handleImport=async()=>{ if(!rows.length)return; setLoading(true); const p=rows.map(r=>({name:r["Ad Soyad"]||r["ad soyad"]||r["AD SOYAD"]||r["name"]||r["Name"]||r["full_name"]||r["first_name"]||"",phone:String(r["Telefon"]||r["telefon"]||r["TELEFON"]||r["phone"]||r["Phone Number"]||""),kurum:r["Kurum"]||r["kurum"]||r["KURUM"]||r["school"]||"",phone:String(r["Telefon"]||r["phone"]||r["Phone Number"]||""),kurum:r["Kurum"]||r["kurum"]||"",status:"yeni",sinav_tipi:[],ogrenci_sayisi:0,user_id:userId})).filter(r=>r.name||r.phone); const{error}=await supabase.from("leads").insert(p); if(error)setError("Hata: "+error.message); else{setDone(true);onImport();} setLoading(false); };
  return (
